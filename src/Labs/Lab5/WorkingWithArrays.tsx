...
<a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
    ...
    <Form.Check type="checkbox" defaultChecked={todo.completed} className="float-start me-2"
                onChange={(_e) => setTodo({ ...todo, completed: !todo.completed }) }/>
...
<a href={`${API}/${todo.id}/description/${todo.description}`} className="btn btn-primary float-end">
    ...
    <FormControl type="text" value={todo.description} className="float-start me-2"
